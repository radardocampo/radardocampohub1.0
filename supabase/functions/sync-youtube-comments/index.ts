...
// hitting the YouTube API on every page load. Replying is a separate edge
// function (reply-youtube-comment) that writes live to YouTube.
serve(async (req) => {
  ...
  try {
    ...
    // 2. Page through commentThreads for the whole channel (bounded so a single
    //    run can't spiral into hundreds of API calls on a very active channel).
    const MAX_PAGES = 10; // up to ~1000 threads per sync
    ...
    do {
      ...
      if (!res.ok) {
        ...
        if (res.status === 403) {
          console.warn("commentThreads.list 403 (comments may be disabled):", JSON.stringify(body));
          break;
        }
        throw new Error(`commentThreads.list error: ${JSON.stringify(body)}`);
      }
    ...
    await supabase.from("sync_logs").insert({
      platform_id: "youtube",
      status: "success",
      message: `[comments] threads=${commentRows.length}, replies=${replyRows.length}`,
      run_at: new Date().toISOString(),
    });

    return new Response(
      JSON.stringify({ success: true, threads: commentRows.length, replies: replyRows.length }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" } },
    );
  } catch (error: unknown) {
